...
- Для отправки приоритетно используется **WEBM видео** (`.../0_640_N.webm`) через `sendVideo`,
  затем fallback на GIF (`sendAnimation`) и только потом текст.
- По дате выбирается детерминированно разный элемент из списка кандидатов.
- В Telegram отправляется как анимация (`sendAnimation`), при ошибке — как фото/текст fallback.

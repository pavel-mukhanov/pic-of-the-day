...
- Для отправки используется **только WEBM видео** (`.../0_640_N.webm`) через `sendVideo`.
- Если отправка видео не удалась, бот отправляет только текст со ссылкой (без `sendAnimation`/`sendPhoto`).
- По дате выбирается детерминированно разный элемент из списка кандидатов.
- В Telegram отправляется как видео (`sendVideo`), при ошибке — текстовый fallback.
